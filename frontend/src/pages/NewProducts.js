    ...
    <div className='p-5 mt-12'>
        <form className='m-auto w-full max-w-md shadow flex flex-col p-4 bg-white ' onSubmit={handleSubmit}>
            ...
            <select className='bg-slate-200 p-1 my-1' id='category' name='category' onChange={handleOnChange} value={data.category}>
                ...
                <option value={"groccerry"}>Groccerry</option>
            ...
            <label htmlFor='image'>Image
                <div  className='h-40 w-full bg-slate-200  rounded flex items-center justify-center cursor-pointer'>
                    {
                        data.image ? <img src={data.image} alt='newproduct' className="h-full" /> : <span className='text-5xl'><BsCloudUpload/></span> 
                    }
            ...
            <button className='bg-red-500 hover:bg-red-600 text-white text-lg font-medium my-3 h-10 drop-shadow' >Save</button>
        </form>
    </div>
  )
}
